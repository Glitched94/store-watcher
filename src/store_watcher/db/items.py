...
def save_items(items: Dict[str, Dict[str, Any]], db_path: Path) -> None:
    ...
    ) -> tuple[str, str, str, str, str, str, str, Optional[int], str, int, str, str]:
    ...
    rows: Iterable[tuple[str, str, str, str, str, str, str, Optional[int], str, int, str, str]] = (
    ...
    with connect(db_path) as conn:
        conn.executemany(
            """
            INSERT INTO items (key, host, code, url, name, price, availability_message, available, first_seen, status, status_since, image)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
